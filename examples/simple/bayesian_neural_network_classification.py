...
def build_toy_dataset(N=500):
  x = np.linspace(0.1, 3, num=N)
  y = np.zeros(N)
  y[:N/2] = 1
...
inputs = np.linspace(0.1, 5, num=400, dtype=np.float32)
x = tf.expand_dims(inputs, 1)
mus = tf.stack(
    [Bernoulli(logits=neural_network(x, qW_0.sample(), qW_1.sample(),
